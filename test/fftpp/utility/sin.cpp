...
TEST_CASE("Значения синуса, требуемые для первообразных корней из единицы, не отличаются от "
    "библиотечных функций с точностью до машинного эпсилон")
{
    ...
    for (auto degree = std::uint64_t{2}; degree <= (std::uint64_t{1} << 62u); degree *= 2)
    {
        ...
        CHECK(fftpp::sin(angle) == doctest::Approx(std::sin(angle)).epsilon(eps));
    }
}
